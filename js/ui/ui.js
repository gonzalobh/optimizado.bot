...
'uploadAvatar','deleteAvatar','fontSelect','tgChatVisible','tgLeadCapture','tgChatHeaderVisible','hotelName',
...
const deleteBtn = $("deleteAutoResponse");
  ...
  const applyTypeDefaults = (item) => {
    if (!item) return;
    if (!item.extras) item.extras = { title: "", subtitle: "", image: "", buttons: [] };
    if (item.type === "cards") {
  ...
  if (typeSelector && typeSelector.previousElementSibling?.tagName === "LABEL") {
    ...
    labelEl.textContent = t("Response type");
    registerTranslationTarget(labelEl, "Response type");
  }
  if (typeSelector) {
    ["text", "cards", "menu"].forEach(val => {
      const opt = typeSelector.querySelector(`option[value="${val}"]`);
      if (!opt) return;
      const key = val === "cards" ? "Cards" : val === "menu" ? "Menu" : "Text";
  ...
  typeModalOptions.forEach(option => {
    ...
    const key = type === "cards" ? "Cards" : type === "menu" ? "Menu" : "Text";
    if (titleEl) {
      titleEl.textContent = t(key);
      registerTranslationTarget(titleEl, key);
    }
    const descKey = type === "cards" ? modalCopy.descriptions.cards : type === "menu" ? modalCopy.descriptions.menu : modalCopy.descriptions.text;
    if (descEl) {
...
topRow.className = "flex items-center gap-2";
    ...
    const badgeKey = item.type === "cards" ? "Cards" : item.type === "menu" ? "Menu" : "Text";
...
titleInput.dataset.cardIndex = idx;
...
linkInput.dataset.cardIndex = idx;
